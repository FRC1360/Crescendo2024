...
public class ShintakePivotSubsystem extends SubsystemBase {
    ...
    public void resetMotorRotations() {
        double newPos = (this.absoluteEncoder.getAbsolutePosition() - Constants.STPConstants.ShintakePivot_ENCODER_OFFSET) / Constants.STPConstants.ShintakePivot_GEAR_RATIO;
    ...
    // Returns the ShintakePivot GLOBAL angle. The global angle is the angle relative to the shoulder
    ...
    public void setShintakePivotNormalizedVoltage(double voltage) {
        this.setShintakePivotVoltage(voltage * 12.0);  // Should probably change this to a constant somewhere for ARM_VOLTAGE
    }

    // This return a GLOBAL angle. The global angle is the angle relative to the shoulder
    /*public double getTargetAngle() {  // Use getTargetAngle() when doing commands to move the ShintakePivot
        
        return -this.shoulderShintakePivotMessenger.getShoulderAngle() + this.getShintakePivotOffset() + (manualOffsetEnable.getAsBoolean() ? -manualOffset.getAsDouble() : 0.0);
    }*/
    ...
    // The offset is more akin to a LOCAL angle. The local angle is the angle relative to ShintakePivot starting position.
    // This angle ensures the ShintakePivot doesn't move relative to the starting location while the shoulder rotates
    // For example, if the ShintakePivot starts pointing directly up and the ShintakePivot offset is 0, the ShintakePivot will stay pointing up
    // regardless of the shoulder's orientation.  Change this value when you want to change the angle of the ShintakePivot
    public double getShintakePivotOffset() {
        return this.ShintakePivotOffset;
    }

    // The CACHE is a means of saving an arbitrary ShintakePivot position to return to later
    // Setting the cache offset saves the current angle
    // Getting the cache offset gets the angle that the ShintakePivot was at when it was last set
    ...
    public void updateSmartDashboard() {
        ...
        // SmartDashboard.putNumber("ShintakePivot_Cache_Offset", this.getCacheOffset());
        // SmartDashboard.putNumber("ShintakePivot_Manual_Offset", this.manualOffset.getAsDouble());
