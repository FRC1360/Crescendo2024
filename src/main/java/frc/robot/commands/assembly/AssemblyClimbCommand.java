...
public class AssemblyClimbCommand extends SequentialCommandGroup {
        public AssemblyClimbCommand(ArmChassisPivotSubsystem ACPSubsystem,
                        ShintakePivotSubsystem STPSubsystem, LEDSubsystem ledSubsystem,
                        ClimberSubsystem climberSubsystem, StateMachine sm) {
                addCommands(
                                ...
                                new STPGoToPositionCommand(STPSubsystem, Constants.CLIMB_POSITION_STP, ACPSubsystem)
                                                .alongWith(new InstantCommand(() -> SmartDashboard
                                                                .putString("Climb stage", "STAGE 2")))

                                                .alongWith(
                                                                // Command 2
                                                                new ACPGoToPositionCommand(ACPSubsystem,
                                                                                Constants.CLIMB_POSITION_ACP,
                                                                                STPSubsystem)
                                                                                .alongWith(new InstantCommand(
                                                                                                () -> SmartDashboard
                                                                                                                .putString("Climb stage",
                                                                                                                                "STAGE 3")))),
                                // Command 3 add climber stuff here
                                new InstantCommand(() -> climberSubsystem.setTargetHeight(
                                                Constants.ClimbConstants.CLIMBER_ENCODER_CLIMBED_HEIGHT_IN_ROTATIONS)),
